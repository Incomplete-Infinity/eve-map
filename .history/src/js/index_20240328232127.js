...
const Universe = {
  ...
  initializer: {
    ...
    createJump: function (sys1, id2) {
      var sys2 = Universe.getObjectById(id2);
...
const Dispobj = {
  ...
  initializer: {
    ...
    createConstellationData: (data) => {
      var con = Dispobj.initializer.basicData(data);
  ...
  getLabelBounds: function (data, scale) {
    ...
    var hWidth = Math.ceil(data.name.length * scale * 0.4),
...
const Gradata = {
  ...
  getRegionMode: (data) => {
    var hsl = Gradata.stringToColor(data.region.name, data.region);

    var reg = data.region.name;
  ...
  getConstellationMode: (data) => {
    var hsl = Gradata.stringToColor(
      data.constellation.name,
      data.constellation
    );

    var con = data.constellation.name;
  ...
  getSovColor: (sys) => {
    var sov = Gradata.getSovEntry(sys);
  ...
  getPirateText: (sys) => {
    ...
    if (!rat && Helper.isAnoikis(sys)) rat = "Sleeper";
  ...
  getBorderColor: (sys) => {
    if (Helper.isAnoikis(sys)) return "hsl(270,50%,60%)";
    if (sys.security <= 0 && Helper.isNPCSpace(sys)) return "hsl(0,50%,60%)";
  ...
  getEffectColor: (id) => {
    var SEL = Universe.getObjectById(id);
  ...
  stringToColor: (input, obj) => {
    ...
    if (Helper.colorCache[input]) return Helper.colorCache[input];

    // add randomness to string
    if (obj && (Helper.isAbyssal(obj) || Helper.isAnoikis(obj))) {
      str += Helper.abc.charAt(Math.abs(Math.round(obj.position.x * 256)) % 64);
      str += Helper.abc.charAt(Math.abs(Math.round(obj.position.y * 987)) % 64);
      str += Helper.abc.charAt(Math.abs(Math.round(obj.position.z * 187)) % 64);
    }
    ...
    Helper.colorCache[input] = output;
  ...
  stringNoise: (input) => {
    ...
    for (i = 0; i < 12; i++) {
      if (!(i % 2)) {
        if (a < input.length) {
          c = Helper.abc.charAt(
            (Helper.abc.indexOf(input.charAt(a)) + Math.round(Math.sqrt(b))) %
              64
          );
          a++;
        } else c = Helper.abc.charAt(b % 64);
      } else c = Helper.abc.charAt(b % 64);

      b += (i + a) * (input.length + 7) + Helper.abc.indexOf(c);
      output += c;
      c = "";
    }
    return output;
...
const Viewport = {
  ...
  init: (mode) => {
    ...
    Viewport.canvas._ctx = Viewport.canvas.getContext("2d");
...
const Camera = {
  ...
  focusTo: (data) => {
    var obj = Dispobj.table[data] || false;
...
const Interaction = {
  ...
  zoomCamera: (e) => {
    var times = Math.round((e.deltaY || e.deltaX) / 100),
      isZoomingIn;
  ...
  // selection tracker
  setMouseNear: (add) => {
    var data = Interaction.mouseNear(true);
  ...
  modifySelection: (data, add) => {
    var index = State.selection.indexOf(data);
  ...
  modifyEditSelection: (data, add) => {
    var index = State.customEditSelection.indexOf(data);
  ...
  cycleEditSelectionType: (e) => {
    var cycle = ["system", "constellation", "region"],
      dir = 1;
...
Layer.systems = {
  ...
  drawIconDot: function (data) {
    ...
    if (data.display.glow) {
      ...
      ctx.lineWidth = State.sysBorderWidth + 16;
      ctx.stroke();
      ctx.lineWidth = State.sysBorderWidth + 12;
      ctx.stroke();
...
Layer.labels = {
  ...
  getAlpha: (type) => {
    if (type == "R") {
      ...
      else if (Camera.scale < 22) return 1;
      else if (Camera.scale < 48) return 0.75;
      else if (Camera.scale < 70) return 0.5;
      else return 0.1;
    }
    if (Camera.scale < 70) {
      return 0.75;
